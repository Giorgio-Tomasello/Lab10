...
public class Model {
	...
	public Model() {
		...
		public List<Flow> getFlows(int i){
			...
			idMap.get(i).setFlows(riversDAO.getAllFlows(i, idMap));
			idMap.get(i).setFlowAvg(getMediaFlow(i));
			return idMap.get(i).getFlows();
		}
		
		public double getMediaFlow(int i) {
			
			double somma=0.0;
			for(Flow f : idMap.get(i).getFlows()) {
			somma += f.getFlow();	
		}
			
			return somma/idMap.get(i).getFlows().size();
		}
